...
public class VerifyTerminalSupportSMPGermanFlow extends AbstractFlow {
    ...
    @Story("Verify elements are correctly visible for German SMP users")
    @Description("Check that every dynamic elements are correctly visible for German SMP users")
    ...
    protected void run(
        ...
        Allure.step("Check the Chat with us element is visible", () -> {
            if (EnvUtil.isAndroid()) {
                driver.waitUntilElementVisible(CHAT_WITH_US_BUTTON.byAndroidXpathWithResourceIdAndAttribute("content-desc", "menu_icon_content_desc"),
                        WAIT_10_S);
            } else if (EnvUtil.isIos()) {
        ...
        if (!user.description().contains("no_physical_terminal")) {
            Allure.step("Check if the Edit receipt and Automatic tip elements are visibile", () -> {
                if (EnvUtil.isAndroid()) {
                    driver.waitUntilElementVisible(EDIT_RECEIPT_TITLE.byAndroidXpathWithResourceIdAndAttribute("text", "Edit receipt text"), WAIT_10_S);
                    driver.waitUntilElementVisible(EDIT_RECEIPT_SUBTITLE.byAndroidXpathWithResourceIdAndAttribute("text", "The printed receipt"), WAIT_10_S);
                    driver.waitUntilElementVisible(AUTOMATIC_TIP_TITLE.byAndroidXpathWithResourceIdAndAttribute("text", "Automatic tip function"), WAIT_10_S);
                    driver.waitUntilElementVisible(AUTOMATIC_TIP_SUBTITLE.byAndroidXpathWithResourceIdAndAttribute("text", "Set it up"), WAIT_10_S);
                } else if (EnvUtil.isIos()) {
                    driver.waitUntilElementVisible(EDIT_RECEIPT_TITLE, WAIT_10_S);
                    driver.waitUntilElementVisible(EDIT_RECEIPT_SUBTITLE, WAIT_10_S);
                    driver.waitUntilElementVisible(AUTOMATIC_TIP_TITLE, WAIT_10_S);
                    driver.waitUntilElementVisible(AUTOMATIC_TIP_SUBTITLE, WAIT_10_S);
                } else {
                    throw new UnsupportedPlatformException();
                }
            });
            // Checks in Terminal details only if the account has physical terminals
            Allure.step("Open Terminal details to check if Chat with us element is visible", () -> {
                if (EnvUtil.isAndroid()) {
                    WebElement terminal_details = driver.waitUntilElementVisible(TERMINAL_LOCATION.byAndroidXpathWithResourceIdAndIndex(5, true), WAIT_10_S);
                    terminal_details.click();
                } else if (EnvUtil.isIos()) {
                    WebElement terminal_details = driver.waitUntilElementVisible(TERMINAL_LOCATION.byIosXpathWithNameAndIndex("Arrow_Right", 5), WAIT_10_S);
                    terminal_details.click();
                } else {
                    throw new UnsupportedPlatformException();
                }
                driver.waitUntilElementVisible(TERMINAL_CHAT_BUTTON, WAIT_10_S);
            });
        } else {
            Allure.step("Check if the Edit receipt and Automatic tip elements are NOT visibile", () -> {
                if (EnvUtil.isAndroid()) {
                    ...
                    driver.waitUntilElementNotVisible(AUTOMATIC_TIP_TITLE.byAndroidXpathWithResourceIdAndAttribute("text", "Automatic tip function"),
                            WAIT_10_S);
