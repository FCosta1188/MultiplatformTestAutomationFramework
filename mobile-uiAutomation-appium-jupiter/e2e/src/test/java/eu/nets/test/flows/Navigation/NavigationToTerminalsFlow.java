...
import static eu.nets.test.core.enums.MpaWidget.LABEL;
...
public class NavigationToTerminalsFlow extends AbstractFlow {
    ...
    @Description("Navigate to Terminals screen using different users")
    ...
    protected void run(
        ...
        Allure.step("Tap TERMINALS button and verify Terminals page layout", () -> {
            driver.safeClick(TERMINALS_DASHBOARD_BUTTON, WAIT_10_S);

            // Check to verify the label "My terminals (x)" is present, without considering the number between parentheses
            WebElement element = driver.findElement(By.xpath("//*[contains(@text, 'My Terminals')]"));
            String actualText = element.getText();
            assert (actualText.matches("My Terminals \\(\\d+\\)"));

            if (EnvUtil.isAndroid()) {
                // Check specific elements for the Terminals screen
                driver.waitUntilElementVisible(NAV_BAR_ACTIVE_ITEM.byAndroidXpathWithResourceIdAndAttribute("text", "TERMINALS"), WAIT_10_S);
                driver.waitUntilElementVisible(SECTION_TITLE.byAndroidXpathWithResourceIdAndAttribute("text", "Terminals"), WAIT_10_S);
                driver.waitUntilElementVisible(LABEL.byAndroidXpathWithResourceIdAndAttribute("text", "Manage your terminals"), WAIT_10_S);
                // Check the inactive elements on the navigation bar
                ...
                driver.waitUntilElementVisible(TITLE.byIosXpathWithName("Terminals"), WAIT_10_S);
                driver.waitUntilElementVisible(TITLE.byIosXpathWithName("Manage your terminals"), WAIT_10_S);
            } else {
                throw new UnsupportedPlatformException();
            }
        });
